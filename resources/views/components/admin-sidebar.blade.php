<aside class="w-80 bg-gradient-to-b from-white to-gray-50 border-r border-gray-200 h-screen flex flex-col sticky top-0 transition-all duration-300 {{ $compact ? 'w-20' : '' }}">
    <!-- Header -->
    <div class="p-6 border-b border-gray-200 bg-white">
        <div class="flex items-center space-x-4">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-400 mt-1">Last login: {{ $user->last_login_at?->diffForHumans() ?? 'Recently' }}</p>
            </div>
            @else
    ...
    <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1">
        ...
        <div class="mb-4">
            ...
            @foreach($systemItems as $item)
                ...
                @if(Route::has($item['route']))
                <a href="{{ route($item['route']) }}" class="{{ $classes }}">
                    <span class="flex-shrink-0 w-8 h-8 flex items-center justify-center text-lg rounded-lg {{ $isActive ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-600 group-hover:bg-gray-200' }}">
                        {{ $item['icon'] }}
                    </span>
                    ...
                    <div class="ml-3 flex-1 min-w-0">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium truncate">{{ $item['label'] }}</span>
                            @if($item['badge'])
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium text-white bg-orange-500">
                                    {{ $item['badge'] }}
                                </span>
                            @endif
                        </div>
